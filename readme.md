...
1. **硬件初始化**  
   - 初始化超声波模块的引脚，配置 Trig 引脚为推挽输出，Echo 引脚为浮空输入。
   - 初始化定时器，用于精确测量超声波信号的传播时间。
   - 初始化串口，用于将测量结果发送到电脑。

2. **超声波测距流程**  
   - 通过 Trig 引脚发送一个 10 微秒的高电平脉冲，触发超声波模块发射超声波。
   - 使用定时器记录 Echo 引脚从低电平变为高电平的时间（超声波发射后返回的起始时间）。
   - 记录 Echo 引脚从高电平变为低电平的时间（超声波返回的结束时间）。
   - 计算起始时间和结束时间的差值，得到超声波的往返时间。

3. **距离计算**  
   - 根据公式 $d = \frac{vt}{2}$ 计算距离，其中 $v$ 为声速，$t$ 为超声波的往返时间。
   - 将时间单位从微秒转换为秒，并代入声速 $v = 340 \, \text{m/s}$ 进行计算。

4. **结果输出**  
   - 判断测量的距离是否在有效范围（5 cm 至 200 cm）内。
   - 通过串口将测量结果输出到电脑，若超出范围则输出 "Out of range!"。

5. **循环测量**  
   - 每次测量后延时 100 毫秒，重复上述步骤，持续测量距离。
